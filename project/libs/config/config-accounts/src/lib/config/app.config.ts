...
export interface AccountApplicationConfig {
  environment: string;
  port: number;
}
...
export default registerAs('application', (): AccountApplicationConfig => {
  ...
  const validationSchema = Joi.object<AccountApplicationConfig>({
  ...
  if (error) {
    throw new Error(
      `[Accounts Application Config]: Environments validation failed. Please check .env file.
